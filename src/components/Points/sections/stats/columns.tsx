...
export const rewardsColumns: ColumnDef<UserPointHistory>[] = [
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="pl-6 whitespace-nowrap flex flex-row items-center gap-x-4">
          <Image
            src="/icons/PnlPositiveIcon.svg"
            alt="Rewards action"
            height={45}
            width={45}
          />
          <div className="flex flex-col gap-y-1 text-left text-sm/5 font-bold pl-6 py-3">
            <p className="text-white opacity-95">Earned {points} Gpoints</p>
            <p className="font-normal text-[#9299AA]">{action} Position</p>
          </div>
        </div>
      );
    }
  ...
  },
  {
    ...
    accessorKey: "date",
    header: () => (
      <div className="pb-4 pr-4 w-full text-center">
        <span>Date</span>
      </div>
    ),
    cell: ({ row }) => {
